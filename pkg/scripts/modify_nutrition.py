...
def parse_args():
    ...
    parser.add_argument("--min-id", type=int, default=0, help="只处理 id 大于等于该值的 recipes")
    parser.add_argument("--max-id", type=int, default=0, help="只处理 id 小于等于该值的 recipes")
    parser.add_argument("--all", action="store_true", help="处理所有 recipes，而不是仅处理缺失营养的记录")
    parser.add_argument("--names-json", default="", help="只处理 JSON 文件中包含的菜名，支持数组对象里的 name 字段")
    parser.add_argument("--only-suspicious-portion", action="store_true", help="只处理推荐份量不在当前餐次合理范围内的记录")
    ...
    parser.add_argument(
        "--min-portion-confidence",
        type=float,
        default=0.0,
        help="低于完整营养回填置信度时，仍允许只回填 portion_weight_g 的最低置信度",
    )
    parser.add_argument("--keep-invalid", action="store_true", help="回填后不软删除仍无营养数据的菜谱")
    return parser.parse_args()


def load_names_from_json(path):
    if not path:
        return []
...
def suspicious_portion_condition():
    return """
    (
        portion_weight_g IS NULL OR portion_weight_g <= 0
        OR energy IS NULL OR energy <= 0
        OR (meal_type = '早餐' AND (portion_weight_g < 180 OR portion_weight_g > 350))
        OR (meal_type = '午餐' AND (portion_weight_g < 250 OR portion_weight_g > 450))
        OR (meal_type = '晚餐' AND (portion_weight_g < 250 OR portion_weight_g > 450))
        OR (meal_type = '加餐' AND (portion_weight_g < 80 OR portion_weight_g > 220))
    )
    """
...
def load_recipes(args):
    ...
    """
    params = []
    names = load_names_from_json(args.names_json)

    if args.recipe_id > 0:
        sql += " AND id = %s"
        params.append(args.recipe_id)
    elif args.min_id > 0 or args.max_id > 0:
        if args.min_id > 0:
            sql += " AND id >= %s"
        ...
        if names:
            ...
            params.extend(names)
        if args.only_suspicious_portion:
            sql += f" AND {suspicious_portion_condition()}"
        elif not args.all:
            sql += """
            AND (
                portion_weight_g IS NULL OR portion_weight_g <= 0
        ...
        if args.only_suspicious_portion:
            sql += f" AND {suspicious_portion_condition()}"
    elif args.only_suspicious_portion:
        sql += f" AND {suspicious_portion_condition()}"
    elif not args.all:
        sql += """
        AND (
...
- 如果这道菜可作为单人一餐，请把 total_weight_g 估算为至少一人份最终可食用重量；不要因为食材写着“适量”就给出偏小份量
...
def portion_range_by_meal_type(meal_type):
    ranges = {
        "早餐": (180.0, 350.0),
        "午餐": (250.0, 450.0),
        "晚餐": (250.0, 450.0),
    ...
    return ranges.get(str(meal_type).strip(), (150.0, 400.0))


def normalize_estimate_portion(recipe, estimate):
    lower, upper = portion_range_by_meal_type(recipe.get("meal_type", ""))
    normalized_portion = max(lower, min(estimate["recommended_portion_weight_g"], upper))
    estimate["recommended_portion_weight_g"] = round(normalized_portion, 2)
    if estimate["total_weight_g"] < normalized_portion:
        estimate["total_weight_g"] = round(normalized_portion, 2)
    return estimate


def normalize_portion_by_meal_type(meal_type, portion_weight_g, total_weight_g):
    lower, upper = portion_range_by_meal_type(meal_type)
...
def classify_recipe(recipe, nutrition):
    ...
    alcohol_keywords = [
        "鸡尾酒", "威士忌", "伏特加", "朗姆", "金酒", "利口酒", "啤酒", "红酒", "白酒",
        "莫吉托", "mojito", "长岛冰茶", "玛格丽塔",
    ]
    ...
    is_general = not has_alcohol

    is_weight_loss = (
        is_general
        and not contains_any(content, high_sugar_drink_keywords + fried_heavy_keywords)
        and energy <= 220
        and fat <= 12
        and carbohydrate <= 25
    )
    ...
    is_sugar_control = (
        is_general
        and not contains_any(content, high_sugar_drink_keywords)
        and carbohydrate <= 20
        and energy <= 230
    )
...
def update_recipe_portion(recipe_id, portion_weight_g):
    sql = """
    UPDATE recipes
    SET
        portion_weight_g = %s,
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(sql, (portion_weight_g, recipe_id))
        conn.commit()
    finally:
        conn.close()
...
def process_recipe(client, recipe, args):
    ...
    for _ in range(args.retry):
        try:
            ...
            estimate = normalize_estimate_portion(recipe, estimate)
            estimate = apply_reasonableness_rules(recipe, estimate)
            if estimate["confidence"] < args.min_confidence:
                portion_weight = normalize_portion_by_meal_type(
                    recipe.get("meal_type", ""),
                    estimate["recommended_portion_weight_g"],
                    estimate["total_weight_g"],
                )
                if estimate["confidence"] >= args.min_portion_confidence:
                    portion_nutrition = {"portion_weight_g": round(portion_weight, 2)}
                    if not args.dry_run:
                        update_recipe_portion(recipe["id"], portion_nutrition["portion_weight_g"])
                    return {
                        "ok": True,
                        "portion_only": True,
                        "recipe_id": recipe["id"],
                ...
                return {
                    ...
                    "name": recipe["name"],
                    "reason": (
                        f"模型置信度过低: {estimate['confidence']} "
                        f"< portion floor {args.min_portion_confidence}"
                    ),
                    "estimate": estimate,
                }
        ...
        with ThreadPoolExecutor(max_workers=max(1, args.workers)) as executor:
            ...
            for future in as_completed(futures):
                ...
                if result["ok"]:
                    ...
                    if result.get("portion_only"):
                        action = "estimate" if args.dry_run else "update"
                        tqdm.write(
                            f"[{result['recipe_id']}] {result['name']} {action} portion only | "
                            f"portion={nutrition['portion_weight_g']}g | "
                            f"conf={estimate['confidence']} < nutrition floor {args.min_confidence}"
                        )
                        success += 1
                        pbar.update(1)
                        continue
